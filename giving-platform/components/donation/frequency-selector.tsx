...
export function FrequencySelector({ value, onChange, disableRecurring = false }: FrequencySelectorProps) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {frequencies.map((freq) => {
        const isRecurring = freq.value !== "one-time";
        const isDisabled = disableRecurring && isRecurring;

        return (
          <button
            key={freq.value}
            type="button"
            onClick={() => !isDisabled && onChange(freq.value)}
            disabled={isDisabled}
            className={cn(
              ...
              isDisabled && "opacity-50 cursor-not-allowed hover:border-slate-200 hover:bg-white"
            )}
            title={isDisabled ? "Recurring payments require credit card (max $10,000)" : undefined}
          >
