...
function easeInOutCustom(t: number): number {
  // First 50%: very slow start so user can focus on the heart
  // Middle 35%: rapid acceleration
  // Last 15%: slight deceleration for smooth landing
  if (t < 0.5) {
    // Slow start - cubic ease in, scaled to 0-0.1 output (barely moves)
    const localT = t / 0.5;
    return 0.1 * (localT * localT * localT);
  } else if (t < 0.85) {
    // Rapid acceleration phase - maps 0.5-0.85 input to 0.1-0.9 output
    const localT = (t - 0.5) / 0.35;
    return 0.1 + 0.8 * (localT * localT); // Quadratic for acceleration feel
  } else {
    // Smooth landing - ease out for final approach
    const localT = (t - 0.85) / 0.15;
    return 0.9 + 0.1 * (1 - Math.pow(1 - localT, 2));
  }
}
...
function FloatingHeartAnimation({ startPosition, onComplete }: FloatingHeartProps) {
  ...
  React.useEffect(() => {
    ...
    const duration = 1200; // ms - extended for slow start then fast finish
